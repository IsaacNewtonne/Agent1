...
mod integration {
    ...
    async fn db_session_crud() {
        ...
        let session = store.create_session_shell("session_test_agent", Some("Test Session".to_string())).await.unwrap();
        assert_eq!(session.root_agent_id, "session_test_agent");
        
        let loaded = store.get_session(&session.id).await.unwrap();
        assert_eq!(loaded.id, session.id);
        
        store.update_session_status(&session.id, agent1_core::SessionStatus::Completed).await.unwrap();
    ...
    async fn db_events_and_messages() {
        ...
        let session = store.create_session_shell("test_agent", None).await.unwrap();
        
        store.save_message(&agent1_core::Message {
            id: new_id("msg"),
        ...
        store.save_event(&agent1_core::RuntimeEvent {
            id: new_id("evt"),
    ...
    async fn db_mcp_server_crud() {
        ...
        store.update_mcp_server_enabled("test_mcp", false).await.unwrap();
